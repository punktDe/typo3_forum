jQuery(document).ready(function ($) {
	...
	*/
	...
	if (typeof currentPageUid !== 'undefined') {
		typo3_forum_ajaxUrl = window.location.protocol + '//' + window.location.host + window.location.pathname + "?ajaxforumapi=main&pid=" + currentPageUid + "&language=de";
	}
		...
		$.ajax({
			...
			success: function (data) {
				...
				$('.tx-typo3forum-helpfull-btn').click(function () {
					...
					$.ajax({
						type: "GET",
						url: window.location.protocol + '//' + window.location.host + window.location.pathname + "?ajaxforumapi=" + action + "&pid=" + currentPageUid + "&language=de&post=" + $(this).attr('data-post'),
						async: false,
